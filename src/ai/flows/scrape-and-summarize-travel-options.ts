...
const travelOptionsPrompt = ai.definePrompt({
  ...
  prompt: `You are an expert travel assistant. Your task is to find and summarize travel options that provide the best value for money, based on the user's preferences. Provide real, verifiable examples.
  ...
  Summarize available travel options (flights, trains, buses) to {{{destination}}} for the given dates and budget. Prioritize options that offer the best value. Include relevant links and specific carrier names (e.g., specific airlines or train companies).
...
const accommodationOptionsPrompt = ai.definePrompt({
  ...
  input: {schema: ScrapeAndSummarizeTravelOptionsInputSchema},
  output: {schema: SummaryOutputSchema},
  prompt: `You are an expert travel assistant. Your task is to find and summarize accommodation options that provide the best value for money, based on the user's preferences. Use specific, real-world hotel names. Do not use generic phrases like "a luxury hotel".
  ...
  Summarize available accommodation options (hotels, hostels, rentals) in {{{destination}}} for the given dates and budget. Recommend 2-3 specific, real-world hotels with names that are known for good value. Include relevant links and details.
...
const attractionOptionsPrompt = ai.definePrompt({
  ...
  input: {schema: ScrapeAndSummarizeTravelOptionsInputSchema},
  output: {schema: SummaryOutputSchema},
  prompt: `You are an expert travel assistant. Your task is to find and summarize tourist attractions, points of interest and restaurants that provide the best value, based on the user's preferences. Use specific, real-world names.

  Destination: {{{destination}}}
  Interests: {{{interests}}}

  Identify and summarize tourist attractions, points of interest, and restaurants in {{{destination}}} based on the user's interests. Recommend specific, real-world places with names, prioritizing free activities or those with high value. Do not use generic phrases like "a local restaurant". Include relevant links and details.
